...
import { formatCurrency, formatRelativeTime, ORDER_STATUS_LABELS } from '@repo/shared'
import {
  Button, Card, colors, EmptyState, ErrorState, fontSizes, fontWeights,
  Input, Skeleton, spacing, StatusBadge,
} from '@repo/ui'
import { PageShell } from '../../../components/PageShell'


const ALL_STATUSES = ['pending', 'accepted', 'preparing', 'ready', 'completed', 'rejected', 'cancelled'] as const
    ...
    >
      ...
      <View style={styles.filters}>
        <Pressable
          ...
          onPress={() => { setStatusFilter(undefined); setPage(1) }}
        >
          <Text style={[styles.chipText, !statusFilter ? styles.chipTextActive : null]}>All</Text>
        </Pressable>
        {ALL_STATUSES.map((s) => (
          <Pressable
            key={s}
          ...
          >
            <Text style={[styles.chipText, statusFilter === s ? styles.chipTextActive : null]}>
      ...
      {isError ? (
        ...
        <EmptyState
          icon="📋"
          title="No orders"
          description={statusFilter ? `No ${ORDER_STATUS_LABELS[statusFilter]?.toLowerCase()} orders` : 'No orders yet'}
        />
      ) : (
        <FlatList
          data={orders}
              ...
              <Pressable style={({ pressed }) => [styles.row, pressed ? styles.rowPressed : null]}>
                <View style={styles.rowLeft}>
                  ...
                  <Text style={styles.rowMeta}>
                    {(item as any).customerName ?? 'Walk-in'} · {(item as any).itemCount} item{(item as any).itemCount !== 1 ? 's' : ''} · {formatRelativeTime(item.createdAt)}
                  </Text>
                </View>
                <View style={styles.rowRight}>
                  <Text style={styles.rowTotal}>{formatCurrency(item.totalCents)}</Text>
      ...
      {total > 25 && (
        <View style={styles.pagination}>
          ...
          <Text style={styles.pageInfo}>Page {page} of {Math.ceil(total / 25)}</Text>
          <Button
            variant="secondary"
            size="sm"
...
const styles = StyleSheet.create({
  ...
  orderId: { fontSize: fontSizes.md, fontWeight: fontWeights.bold as any, color: colors.textPrimary, fontFamily: 'monospace' },
  rowMeta: { fontSize: fontSizes.sm, color: colors.textSecondary },
  rowRight: { flexDirection: 'row', alignItems: 'center', gap: spacing[2] },
  rowTotal: { fontSize: fontSizes.md, fontWeight: fontWeights.semibold as any, color: colors.textPrimary },
